...
Sistema de gestión frontend desarrollado dinámicamente con JavaScript puro (Vanilla JS), CSS3 y HTML5 sin uso de frameworks ni librerías externas. La aplicación permite organizar rutas escolares, gestionar asignaciones de estudiantes, actualizar información en tiempo real y consumir servicios de APIs públicas.
...
* **Manipulación Dinámica del DOM:** Creación, edición y eliminación de rutas y estudiantes sin recargar la página.
* **Componentes Reutilizables (Web Components):** Implementación del custom element `<route-card>` encapsulado mediante Shadow DOM y plantillas HTML (`<template>`).
* **Eventos Personalizados (`CustomEvent`):** Comunicación bidireccional entre los Web Components y la lógica principal del sistema.
* **Consumo de APIs Públicas (`fetch` y `async/await`):**
  * **Random User Generator API:** Carga de perfiles aleatorios para conductores y estudiantes.
  * **Open-Meteo API:** Muestra en tiempo real la temperatura y el clima para la ciudad donde opera la ruta (San Gil, Santander).
* **Validación de Formularios:** Control de campos obligatorios e indicación visual mediante un módulo independiente.
* **Diseño Responsivo (3 Breakpoints):**
  * **Mobile:** `< 640px` (Vista apilada en una sola columna)
  * **Tablet:** `641px - 1024px` (Formularios y tarjetas en grilla de 2 columnas)
  * **Desktop:** `> 1025px` (Grilla principal de 3 columnas)
...
└── js/
    ├── app.js            # Controlador principal y gestión del estado
    ├── RouteCard.js      # Web Component <route-card> (Shadow DOM)
    ├── api.js            # Servicio de consumo de APIs públicas (RandomUser & Clima)
    └── validators.js     # Funciones para la validación de formularios